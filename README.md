...
> 面向浏览器书签的可搜索、多级分类、可导入导出单页导航；支持本地优先使用，以及登录后的云端 JSON 备份与恢复。
...
| 导入 | 支持标准书签 JSON、极光Tab/WebDesk 原始 JSON、浏览器 HTML、XLSX 与 CSV；表格可按分类路径还原多级目录 | `bookmarks.ts` |
| 导入预览 | 文件解析后先展示文件名、书签数、分类数、目录深度和书签样本，再确认覆盖或增量写入 | `Home.tsx`、`bookmarks.ts` |
...
| 独立数据工具 | 顶部“数据工具”直接提供四类文件导入、五类导出与 XLSX/CSV 空白模板下载 | `Home.tsx`、`index.css` |
...
如果只想使用网站，请依次阅读第 3、6、7 节；如果准备更改默认书签数据，请继续阅读第 4、5 节；如果需要部署或启用云端备份，请阅读第 8、9 节。第 10 节给出所有验证命令，第 11 节整理常见排错入口。
...
浏览器端的 `Home.tsx` 负责显示和管理当前书签树；它优先读写浏览器 LocalStorage。导入、导出和分类处理都复用 `client/src/lib/` 中的纯函数。登录后的云端备份请求通过 tRPC 进入 `server/routers.ts`，JSON 正文进入受管理对象存储，元数据进入 MySQL。外部备份同样只由服务端发起，浏览器只获得“已配置/待配置”状态，永远不会接触密码或令牌。

```text
浏览器页面（Home.tsx）
  ├─ LocalStorage：当前书签、图标来源、清除标记
  ├─ client/src/lib：解析、合并、表格、离线导出、管理树操作
  └─ tRPC（登录后）→ server/routers.ts
                         ├─ 对象存储：备份 JSON 正文
                         ├─ MySQL：用户、备份元数据
                         └─ externalBackups.ts：WebDAV / KV / D1 服务端请求
```
...
| `client/src/components/ui/dialog.tsx` | 设置和数据工具使用的可访问 Dialog 组件，含焦点管理与 Escape 关闭 | 修改模态交互前先阅读 |
| `server/externalBackups.ts` | 坚果云 WebDAV、Cloudflare KV/D1 Worker 代理的服务端备份适配器 | 改外部备份目标或请求格式 |
| `server/externalBackups.test.ts` | 外部备份配置状态、请求鉴权和环境变量解析测试 | 改变量名或外部适配器后必须运行 |
| `external-backup-research.md` | 坚果云 WebDAV、Cloudflare KV/D1 的官方调研结论与安全边界 | 配置外部备份前阅读 |
| `docs/external-backup-environment.example.md` | 不含真实凭据的环境变量模板和填写说明 | 配置 WebDAV、KV 或 D1 前复制变量名 |
| `drizzle/0001_create_users.sql` | 用户认证表的补充迁移 | 仅供迁移历史参考 |
| `drizzle/0002_dizzy_robbie_robertson.sql` | 外部备份设置元数据表的迁移记录 | 仅供迁移历史参考；不保存密码或令牌 |
| `drizzle/meta/` | Drizzle 生成迁移的快照与顺序记录 | 不要手动删除或改名 |
| `server/_core/` | OAuth、tRPC 上下文、Vite 桥接、存储代理等模板基础设施 | 通常不要直接修改 |
| `validation-notes.md` | 页面、数据工具、导入预览和设置面板的验证记录 | 复核历史验证范围 |
...
### 6.1 使用独立“数据工具”与设置备份页

顶部 **数据工具** 是日常导入和导出的最快入口：可选择 JSON、HTML、XLSX 或 CSV 文件，并可下载 XLSX/CSV 空白模板；所有导出集中在同一窗口。**设置 → 备份与恢复** 保留同样的本地恢复能力，并额外提供云端备份、云端恢复、清除和恢复默认数据。

空白表格模板只包含标准列名，不含任何用户书签，可作为从零录入或向同事分发的数据格式样板。

### 6.2 导入 JSON、浏览器 HTML、XLSX 或 CSV

打开顶部 **数据工具 → 选择 JSON / HTML / XLSX / CSV 文件**，或在 **设置 → 备份与恢复 → 从本地文件恢复** 中选择文件。系统会先解析文件并显示摘要预览，再要求选择导入策略；在确认前不会改写当前书签。
...
JSON 可以直接保留完整字段，也兼容此前使用过的极光Tab/WebDesk 原始 JSON 形态；系统会转换为标准树结构后显示。HTML 导入遵循标准浏览器书签结构，目录与网址顺序会被保留。XLSX 和 CSV 使用“每行一个网址”的交换格式，导入时会按“分类路径”自动创建或复用文件夹，因此左侧分类树也会同步生成多级结构。
...
<a id="external-backups"></a>

### 6.7 坚果云与 Cloudflare 外部备份
