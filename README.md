...
> **安全提醒：** 导出操作现在要求受管理的登录会话，并会向服务端再次确认授权；项目不再保存或校验固定前端账号密码。默认数据仍会作为公开静态文件发布；不要将私密链接、账号、密码或令牌写入默认 JSON、前端源码或浏览器本地存储。
...
| 导出 | JSON、浏览器 HTML、XLSX、CSV、离线单页导航 HTML；主站下载需登录会话授权 |
...
JSON、浏览器书签 HTML、XLSX、CSV 与离线单页导航 HTML 的下载均由当前站点登录会话授权。点击导出时，前端会先检查登录状态，再调用受保护的服务端授权接口；未登录时会跳转至既有登录流程。

离线单页导航为了避免伪造安全验证，不再内置固定账号密码，也不能验证服务器会话。它保留目录、搜索、主题、图标和导入等本地体验；需要导出数据时，请回到已登录的主站完成操作。

这项机制保护的是受管理部署中的下载入口；浏览器 LocalStorage 与公开默认 JSON 本身不适合存放私密书签或任何凭据。
