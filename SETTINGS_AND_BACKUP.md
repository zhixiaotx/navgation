...
| 备份与恢复 | 下载 JSON/XLSX/CSV、本地恢复、云端备份、云端恢复、恢复默认、一键清除 | 覆盖、清除、恢复默认均要求确认；主站导出需登录会话授权 |
...
> **导出授权边界：** 主站的 JSON、HTML、XLSX、CSV 与离线单页导航下载会先核验当前登录会话，再由受保护的服务端接口确认授权。离线单页导航不能验证服务器会话，因此不会提供受保护的数据导出；请返回已登录主站完成下载。
