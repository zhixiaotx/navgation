...
| 备份与恢复 | 下载 JSON/XLSX/CSV、本地恢复、云端备份、云端恢复、恢复默认、一键清除 | 覆盖、清除、恢复默认均要求确认；数据工具导出需本地账号验证 |
...
> **导出验证边界：** JSON、HTML、XLSX、CSV 与离线单页导航下载使用 `admin / 123456` 本地验证。验证成功状态只保存在当前浏览器，数据工具中的“退出登录”可立即清除状态。该方式不是保护公开数据的严格安全边界；不要把私密书签或敏感凭据写入默认数据。
